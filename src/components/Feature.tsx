...
export const Feature = ({ title, description }: { title: string; description: string }) => {
  ...
  const maskImage = useMotionTemplate`radial-gradient(100px 100px at ${offsetX}px ${offsetY}px, black, transparent)`; // Correct template literal
  ...
  return (
    <div className="border border-white/30 px-5 py-10 text-center rounded-xl sm:flex-1 relative">
      ...
      <p className="mt-2 text-white/70 text-xl">{description}</p>
    </div>
  );
};
